...
watch(selectAndSearch, debounce(function() {
    router.get(route('brand.index'), selectAndSearch, {
        preserveState: true,
        preserveScroll: true
    });
...
function edit($slug){
    axios.get(route('brand.edit', {'slug': $slug}))
    .then(res => {
        if(res.data){
...
function update(){
    editForm.post(route('brand.update', {'id' : editForm.id}), {
        onSuccess: () => {
            editForm.reset()
...
function destroy($id){
    router.delete(route('brand.destroy', {'id' : $id}),{
        preserveScroll:true,
    });
}
...
<template>
    ...
    <MasterLayout>
                ...
                <div class="flex  flex-col">
                    <div class="-m-1.5 overflow-x-auto">
                        <div class="p-1.5 min-w-full inline-block align-middle">
                                ...
                                <table class="table">
                                    ...
                                    <tbody class="divide-y divide-gray-200 dark:divide-neutral-700">
                                        <tr v-for="(brand, index) in brands.data" :key="index" >
                                            ...
                                            <td class="table-td">
                                                <div v-if="brand.image" class="w-[60px] h-[60px] overflow-hidden rounded-xl border-2 border-[#6FD943]">
                                            ...
                                            <td class="table-td">
                                                <span v-if="brand.status == 1" class="inline-flex items-center gap-x-1.5 py-1.5 px-3 rounded-full text-xs font-medium border border-teal-500 text-teal-500">Active</span>
                                            ...
                                            <td class="table-td flex gap-4 items-center h-[90px]">
                                                <button @click="edit(brand.slug)" class="px-2 py-1.5 text-[17px] text-green-500 border border-green-500 rounded-md duration-200 hover:bg-[#6FD943] hover:text-white">
                                                    <EditIcon/>
                                                </button>

                                                <button @click="destroy(brand.id)" class="px-2 py-1.5 text-[17px] text-red-500 border border-red-400 rounded-md duration-200 hover:bg-red-400 hover:text-white">
                                                    <DeleteIcon/>
                                                </button>
                                            </td>
                                        </tr>
                            ...
                            <div class="py-1 flex justify-end mt-2" v-if="brands.data.length > 0">
                                <nav class="flex items-center space-x-1">
                                    <Link v-for="(link, index) in brands.links" :key="index" :href="link.url" v-html="link.label" :class="{'bg-[#6FD943] font-semibold text-white' : link.active}" type="button" class="table-paginate"></Link>
                                </nav>
                            </div>
        ...
        <BrandCreateModalComponent @create="create" />
