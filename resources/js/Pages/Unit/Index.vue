...
watch(selectAndSearch, debounce(function() {
    router.get(route('unit.index'), selectAndSearch, {
        preserveState: true,
        preserveScroll: true
    });
...
function edit($id){
    axios.get(route('unit.edit', {'id': $id}))
    .then(res => {
        if(res.data){
            editForm.name = res.data.name
            editForm.id = res.data.id
            editForm.short_name = res.data.short_name
...
function update(){
    editForm.post(route('unit.update', {'id' : editForm.id}), {
        onSuccess: () => {
            editForm.reset()
...
function destroy($id){
    router.delete(route('unit.destroy', {'id' : $id}),{
        preserveScroll:true,
    });
}
    ...
    <MasterLayout>
        <div class="w-full px-1.5 flex justify-between items-center">
            ...
            <div class="wrapper flex item-center gap-4">
                <button class="btn btn-primary text-[20px] p-1.5 rounded-lg bg-[#6FD943] text-white"  data-hs-overlay="#unit-create-modal">
                    <PlusIcon />
                </button>
            </div>
        </div>
                ...
                <div class="flex  flex-col">
                    <div class="-m-1.5 overflow-x-auto">
                        <div class="p-1.5 min-w-full inline-block align-middle">
                                ...
                                <table class="table">
                                    ...
                                    <tbody class="divide-y divide-gray-200 dark:divide-neutral-700">
                                        <tr v-for="(unit, index) in units.data" :key="index" >
                                            ...
                                            <td class="table-td">{{ unit.short_name }}</td>
                                            <td class="table-td">
                                                <span v-if="unit.status == 1" class="inline-flex items-center gap-x-1.5 py-1.5 px-3 rounded-full text-xs font-medium border border-teal-500 text-teal-500">Active</span>
                                            ...
                                            <td class="table-td flex gap-4 items-center h-[90px]">
                                                <button @click="edit(unit.id)" class="px-2 py-1.5 text-[17px] text-green-500 border border-green-500 rounded-md duration-200 hover:bg-[#6FD943] hover:text-white">
                                                    <EditIcon/>
                                                </button>

                                                <button @click="destroy(unit.id)" class="px-2 py-1.5 text-[17px] text-red-500 border border-red-400 rounded-md duration-200 hover:bg-red-400 hover:text-white">
                                                    <DeleteIcon/>
                                                </button>
                                            </td>
                                        </tr>
                            ...
                            <div class="py-1 flex justify-end mt-2" v-if="units.data.length > 0">
                                <nav class="flex items-center space-x-1">
                                    <Link v-for="(link, index) in units.links" :key="index" :href="link.url" v-html="link.label" :class="{'bg-[#6FD943] font-semibold text-white' : link.active}" type="button" class="table-paginate"></Link>
                                </nav>
                            </div>
        ...
        <UnitCreateModalComponent @create="create" />
        ...
        <div id="unit-edit-modal" class="hs-overlay hidden size-full fixed top-0 start-0 z-[80] overflow-x-hidden overflow-y-auto pointer-events-none">
            <div class="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-0 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto">
                ...
                <div class="flex justify-between items-center py-3 px-4 border-b border-[#6FD943] dark:border-neutral-700">
                    ...
                    <button type="button" class="flex justify-center items-center size-7 text-sm font-semibold rounded-full border border-transparent text-gray-800 hover:bg-gray-100 disabled:opacity-50 disabled:pointer-events-none dark:text-white dark:hover:bg-neutral-700" data-hs-overlay="#unit-edit-modal">
                    ...
                    <div class="p-4 overflow-y-auto">
                        <form @submit.prevent="update" class="space-y-3">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" v-model="editForm.short_name" autofocus class="form-input" placeholder="Enter short name" id="short_name">
                                <span v-if="editForm.errors.short_name" class="form-error" >{{ editForm.errors.short_name }}</span>
                            </div>
